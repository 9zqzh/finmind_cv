...
from app.services.admin import resolve_admin_role, write_audit_safely
...
async def get_captcha(
    ...
    try:
        ...
        await manager.remove(session.token, db)
        raise
...
async def login(
    ...
    try:
        ...
        await write_audit_safely(
            ...
            success=False,
            actor_student_number=payload.username,
            target_student_number=payload.username,
            target_type="user",
            error_code=exc.code,
        )
        raise
    await write_audit_safely(
        ...
        actor_student_number=session.username,
        target_student_number=session.username,
        target_type="user",
    )
...
async def logout(
    ...
    if session is not None:
        ...
        try:
            ...
            await write_audit_safely(
                db, request, event_type="auth.logout", success=False,
                actor_user_id=user_id, actor_student_number=username,
                target_student_number=username, target_type="user",
                error_code=exc.code,
            )
            raise
        await write_audit_safely(
            ...
            actor_student_number=username,
            target_student_number=username,
            target_type="user",
        )

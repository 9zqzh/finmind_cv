...
def main() -> None:
    ...
    with api.GdufClient(timeout=15, retries=2) as client:
        ...
        # 获取竞赛平台中正在报名的比赛
        competitions = api.get_aijspt_bslb(
            year=2026,
            status="registration_open",
            client=client,
        )
        show_result("竞赛平台比赛列表", competitions)

        # 获取比赛详情，传入 CompetitionSummary、UUID 或同域详情 URL
        if competitions.items:
            competition_detail = api.get_aijspt_bsxq(competitions.items[0], client=client)
            show_result("竞赛平台比赛详情", competition_detail)

        # 获取竞赛平台公开通知与社团列表
        competition_notices = api.get_aijspt_tzgg(limit=20, client=client)
        show_result("竞赛平台通知公告", competition_notices)

        clubs = api.get_aijspt_stlb(client=client)
        show_result("竞赛平台社团列表", clubs)
